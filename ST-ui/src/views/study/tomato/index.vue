          ...
          <div class="history-list">
            ...
            >
              ...
              <div class="history-content">
                <div class="history-title">{{ getPlanName(record.planId) || '专注学习' }}</div>
                <div class="history-duration">{{ formatMinutes(record.tomatoDuration || record.duration) }}</div>
                <el-tag
...
export default {
  ...
  methods: {
    ...
    /** 加载设置 - 每次打开页面都重置为默认25分钟 */
    loadSettings() {
      // 每次进入页面都重置为默认值
      this.settings = {
        tomatoDuration: 25, // 默认 25 分钟
        restDuration: 10,   // 默认 10 分钟
        longBreakTime: 15,
        longBreakInterval: 4,
      ...
      // 初始化时间显示
      this.updateTimerDuration();
      console.log('设置已重置为默认值:', this.settings);
    },
    /** 保存设置 */
    saveSettings() {
    ...
    formatTimeOnly(timeStr) {
      if (!timeStr) return '--:--';
      // 如果是 Date 对象或包含 T 的 ISO 格式
      if (typeof timeStr === 'object' || timeStr.includes('T')) {
    ...
    getRecordStatusType(status) {
      ...
      return types[status] || types[String(status)] || 'info';
    },
    // 获取记录状态文本
    getRecordStatusText(status) {
      ...
      return texts[status] || texts[String(status)] || '未知';
    },
    // 根据计划ID获取计划名称
    getPlanName(planId) {
      if (!planId) return null;
      const plan = this.planOptions.find(p => p.planId === planId);
