...
public class TomatoRecordController extends BaseController
{
     ...
     * 查询番茄钟记录列表
     */
    @PreAuthorize("@ss.hasPermi('study:tomato:list')")
    @GetMapping("/list")
     ...
     * 新增番茄钟记录
     */
    @PreAuthorize("@ss.hasPermi('study:tomato:add')")
     ...
     * 启动番茄钟
     */
    @PreAuthorize("@ss.hasPermi('study:tomato:start')")
     ...
     * 中断番茄钟
     */
    @PreAuthorize("@ss.hasPermi('study:tomato:interrupt')")
    @Log(title = "番茄钟", businessType = BusinessType.UPDATE)
