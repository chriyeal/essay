...
export default {
  ...
  data() {
    return {
      ...
      // 效率指标
      efficiencyMetrics: [
        { key: 'focus', label: '专注度', value: 85 },
        { key: 'completion', label: '完成率', value: 78 },
        { key: 'consistency', label: '连续性', value: 92 },
        { key: 'productivity', label: '产出比', value: 88 }
      ],
  ...
  methods: {
    ...
    loadMetrics() {
      getStudyStatistics().then(response => {
        ...
        this.metrics[0].value = this.formatTime(data.totalStudyTime || 0);
        this.metrics[1].value = (data.tomatoCount || 0) + '个';
        this.metrics[2].value = (data.completedTasks || 0) + '个';
        this.metrics[3].value = (data.averageEfficiency || 0) + '分';
      }).catch(() => {
        // 使用模拟数据
        this.metrics = [
          { key: 'totalStudyTime', label: '总学习时长', value: '45小时', icon: 'el-icon-time', color: '#409EFF', change: 12.5 },
          { key: 'tomatoCount', label: '番茄钟总数', value: '86个', icon: 'el-icon-timer', color: '#67C23A', change: 8.3 },
          { key: 'completedTasks', label: '完成任务数', value: '32个', icon: 'el-icon-check', color: '#E6A23C', change: -2.1 },
          { key: 'efficiencyScore', label: '平均效率', value: '85分', icon: 'el-icon-data-analysis', color: '#F56C6C', change: 5.7 }
        ];
      });
    },
